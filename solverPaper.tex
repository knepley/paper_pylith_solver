...
\end{tikzpicture}
where we have introduced a fault gap purely to show explicitly the cohesive cells, which in the simulation have zero volume. In~\cref{fig:sparsityC}, we display the sparsity pattern of our operator highlighting only the $C$ and $C^T$ matrices. The red and blue entries indicate the two sides of the fault, which are distinguished by positive and negative terms. The top panel is the sparsity in the original mesh ordering, whereas the bottom has grouped unknowns coupled by cohesive cells into two blocks.
